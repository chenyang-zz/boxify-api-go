...
- Native App acceptance is executed through the project `.agents/skills/ios-simulator/` skill. The package-owned Maestro workspace under `packages/app/mobile/e2e/maestro/` provides deterministic semantic UI automation for adopted regression flows; other Simulator scenarios remain interactive evidence until encoded in that or another project-owned native framework.
- Current deterministic App coverage includes profile/password persistence, Chat/SSE history restoration, and navigation/native lifecycle behavior across protected routes, keyboard/sheet state, the iOS back gesture, and SecureStore restoration.
...
- Use the implemented workspace-root Make targets: `server-db-smoke`, `e2e-up`, `e2e-app-backend`, `e2e-smoke`, `e2e`, `e2e-logs`, `e2e-down`, `app-mobile-e2e-profile-password`, `app-mobile-e2e-chat-persistence`, and `app-mobile-e2e-native-lifecycle`. Keep `e2e/scripts/e2e.sh` as the dependency lifecycle source of truth and package-owned App drivers under `packages/app/mobile/e2e/maestro/`.
...
- Use Codex Computer Use as the default foreground path for a one-off or exploratory App flow. Use the package-owned Maestro driver for an adopted deterministic regression flow or when repeated accessibility-tree inspection is unavailable. Read current state at each meaningful transition and prefer accessibility targets and stable `testID` values over coordinates in either path.
...
- Use a project-owned XCUITest, Maestro, Detox, or equivalent framework when the scenario must become deterministic CI regression coverage. Cove's adopted Maestro workspace documents Expo SDK 57/React Native 0.86 compatibility, CI requirements, and maintenance cost in `packages/app/mobile/e2e/maestro/README.md`; extend it for suitable high-value flows instead of creating a parallel ad hoc driver.
