...
- (void)loadView {
    ...
    self.view.backgroundColor = CovePageColor();
    if (self.sourceURL) {
...
- (void)covePushNativeRegister {
    ...
    if ([self coveRegisterController].isReady) {
...
- (void)coveNavigationDidShowViewController:(UIViewController *)viewController {
    ...
    BOOL registerVisible = viewController == [self coveRegisterController];
    ...
    if (!registerVisible) {
        [[self coveRegisterController].webView evaluateJavaScript:
