...
func TestHandlerFuncWrapsFunction(t *testing.T) {
	...
	task, err := types.NewParseDocumentTask(uuid.New(), uuid.New())
	if err != nil {
		t.Fatalf("NewParseDocumentTask error = %v", err)
	}
	var called bool
	handler := queue.HandlerFunc(func(ctx context.Context, got *types.Task) error {
		called = got == task
		return nil
	})
...
func TestNewEnqueueOptionsUsesTaskQueueAndOverrides(t *testing.T) {
	...
	opts := queue.NewEnqueueOptions(task, queue.WithQueue(types.QueueBeat), queue.WithMaxRetry(3))
	if opts.Queue != types.QueueBeat {
		t.Fatalf("queue = %q, want %q", opts.Queue, types.QueueBeat)
	}
