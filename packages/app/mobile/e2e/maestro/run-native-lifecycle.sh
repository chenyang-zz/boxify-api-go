...
required_variables=(
  ...
  MAESTRO_E2E_CHAT_DRAFT
  MAESTRO_E2E_UNSAVED_NICKNAME
)
...
if [[ "${MAESTRO_E2E_CHAT_DRAFT}" == "${MAESTRO_E2E_UNSAVED_NICKNAME}" ]]; then
  echo "MAESTRO_E2E_CHAT_DRAFT and MAESTRO_E2E_UNSAVED_NICKNAME must differ." >&2
  exit 2
fi
...
run_id="${E2E_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-native-lifecycle}"
...
recording_path="${artifact_dir}/evidence/native-lifecycle.mp4"
recording_pid=""
...
node "${mobile_dir}/e2e/maestro/setup-user-fixture.mjs"
...
echo "Running Cove navigation/native lifecycle E2E on Simulator ${IOS_SIMULATOR_UDID}"
...
maestro test \
  ...
  "${mobile_dir}/e2e/maestro/flows/native-lifecycle.yaml"
