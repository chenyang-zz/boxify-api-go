...
if [[ "${MAESTRO_E2E_OLD_PASSWORD}" == "${MAESTRO_E2E_NEW_PASSWORD}" ]]; then
  echo "MAESTRO_E2E_NEW_PASSWORD must differ from MAESTRO_E2E_OLD_PASSWORD." >&2
  exit 2
fi

if [[ "${MAESTRO_E2E_WRONG_PASSWORD}" == "${MAESTRO_E2E_OLD_PASSWORD}" || "${MAESTRO_E2E_WRONG_PASSWORD}" == "${MAESTRO_E2E_NEW_PASSWORD}" ]]; then
  echo "MAESTRO_E2E_WRONG_PASSWORD must differ from both valid passwords." >&2
  exit 2
fi
...
run_id="${E2E_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-profile-password}"
...
echo "Running Cove profile/password E2E on Simulator ${IOS_SIMULATOR_UDID}"
...
maestro test \
  ...
  "${mobile_dir}/e2e/maestro/flows/profile-password.yaml"
